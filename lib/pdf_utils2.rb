  ...
  module PdfUtils
    ...
    class PdfWriter
      ...
      def write_family family
        new_page if @block_num > (@blocks_per_page * 0.75)
        str = "#{FamilyDescriptions[family]} (#{family})"
        write_block :heading do
            @pdf.transparent(0.4) do
              # @pdf.line [@pdf.bounds.right, @pdf.bounds.top - 8], [@pdf.bounds.left, @pdf.bounds.top - 8]
            end
            @pdf.pad_top(25) do
      ...
      def write_block type=:bird, &block
        x = case type
            when :bird_row, :major
              0
            when :rareties, :rareties_2x, :heading
